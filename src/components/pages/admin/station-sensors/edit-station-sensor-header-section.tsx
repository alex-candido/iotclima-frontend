// src/components/pages/admin/station-sensors/edit-station-sensor-header-section.tsx
"use client";
...
interface EditStationSensorHeaderSectionProps {
  isLoading: boolean;
  stationSensorId: number | string;
  stationSensor: StationSensor;
}

export function EditStationSensorHeaderSection({
  isLoading,
  stationSensorId,
  stationSensor,
}: EditStationSensorHeaderSectionProps) {
  const { data: session } = useSession();
  const userGroups: UserGroup[] =
    (session?.user?.groupNames as UserGroup[]) || [];
  const router = useRouter();

  const isDeletingStationSensor = false;
  ...
  const pageTitle =
    APP_TEXT.STATION_SENSORS_PAGE.EDIT_TITLE ||
    `Editar Vínculo: ${stationSensor.station_name} - ${stationSensor.sensor_model}`;
  const pageDescription =
    APP_TEXT.STATION_SENSORS_PAGE.EDIT_DESCRIPTION ||
    "Atualize as informações do vínculo estação-sensor.";
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex gap-2">
        {stationSensorId && stationSensor && canEditStationSensor && (
          <Link href={APP_ROUTES.ADMIN.STATION_SENSORS.DETAIL(stationSensorId)}>
            <Button
              variant="outline"
              disabled={isLoading || isDeletingStationSensor}
            >
