...
export async function confirmPasswordReset(data: { token: string; uid: string; new_password1: string; new_password2: string }): Promise<any> {
  const response = await api.post(API_ENDPOINTS.AUTH.PASSWORD_RESET_CONFIRM(data.uid, data.token), data, {
    headers: { 'Content-Type': API_CONTENT_TYPE },
  });
  return response.data;
}
